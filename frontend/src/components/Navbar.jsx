...
export default function Navbar() {
    ...
    return (
        <header className="sticky top-0 z-40 w-full bg-paper/80 backdrop-blur-xl border-b border-line">
            {/* Top Loading Progress Bar */}
            <div className={`absolute top-0 left-0 h-0.5 bg-ink transition-all duration-500 ease-out z-50 ${isNavigating ? 'w-full opacity-100' : 'w-0 opacity-0'}`} />
            ...
            <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
                ...
                <div className="hidden lg:flex items-center gap-0.5">
                    {navLinks.map((link) => (
                        <Link
                            key={link.to}
                            to={link.to}
                            className={`flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-medium transition-all duration-200 ${isActive(link.to)
                                    ? 'bg-ink text-paper'
                ...
                <button
                    ...
                    className="lg:hidden p-2 rounded-full hover:bg-line/60 transition"
                    aria-label="Toggle menu"
                >
                    <svg className="w-6 h-6 text-ink" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.6}>
                        {mobileOpen ? (
                    ...
                    <div className="px-4 py-3 space-y-1">
                        {navLinks.map((link) => (
                            <Link
                                ...
                                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${isActive(link.to)
                                        ? 'bg-ink text-paper'
                                        : 'text-muted hover:text-ink hover:bg-line/60'
                                    }`}
                            >
